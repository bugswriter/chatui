<script lang="ts">
    ...
    $: {
        (async () => {
            const baseHtml = await marked.parse(message.content || "", {
                breaks: true,
                async: false,
            });
            parsedContent = (baseHtml as string).replace(
                /@(\w+)/g,
                (match: string, agentName: string) => {
                ...
                for (const att of message.attachments) {
                    ...
                    if (
                        ...
                        try {
                            ...
                            console.error(
                                `Failed to fetch temp URL for ${att.file_id}:`,
                                e,
                            );
                        }
                    }
    ...
    async function handleDownload(event: CustomEvent<Attachment>) {
        ...
        const url =
            attachment.url ||
            attachmentUrls[attachment.file_id || attachment.s3_key];
        if (url) {
...
{#if isSystem}
    ...
    <div
        class="my-3 flex items-center justify-center gap-3 text-sm text-gray-500"
    >
        <div class="h-px w-full flex-1 bg-chat" />
...
{:else}
    ...
    >
        ...
        {#if !isUser}
            <div
                class="flex h-10 w-10 flex-shrink-0 self-end rounded-full bg-chat shadow-md"
            >
        ...
        >
            <div class="px-1 text-xs font-medium text-gray-500">
                {displayName}
            </div>
                    ...
                    <div class="prose">
                        {#if message.progress}
                            <div class="w-64 text-sm">
                                <p class="font-semibold text-gray-900/90">
                        ...
                        {:else if !message.content && isCurrentlyStreaming}
                            ...
                            <div class="flex items-center gap-1.5">
                                <span
                                    class="h-2 w-2 rounded-full bg-current/60 animate-bounce"
                                ></span>
                                <span
