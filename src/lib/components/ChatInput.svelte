<script lang="ts">
	...
	async function handleSubmit() {
		if (isLoading || isStreaming || (!message.trim() && stagedFiles.length === 0 && reattachedFiles.length === 0))
			return;
		isUploading = true;
		uploadError = null;
		try {
			const uploadedAttachments = await Promise.all(
				stagedFiles.map(async (fileInfo) => {
					...
					Object.entries(fileInfo.uploadData.upload_fields).forEach(([key, value]) =>
					...
					{#each suggestions as agent, i (agent.id)}
						...
						<li>
							...
							>
								...
								/>
								<div class="min-w-0">
									<p class="font-semibold truncate" class:text-primary-foreground={isActive}>
										{agent.name}
									</p>
									<p
		...
		{#if stagedFiles.length > 0 || reattachedFiles.length > 0}
			<div class="mb-3 flex flex-wrap gap-3">
				{#each stagedFiles as file, index (file.filename + file.file.size)}
					...
					>
						{#if file.preview}
							<img src={file.preview} alt={file.filename} class="w-11 h-11 rounded-md object-cover" />
						{:else}
							<div class="flex h-11 w-11 items-center justify-center rounded-md bg-background">
				...
				{#each reattachedFiles as file, index (file.s3_key)}
					...
					>
						<div class="flex h-11 w-11 items-center justify-center rounded-md bg-background/50">
							{#if file.url && file.content_type?.startsWith('image/')}
								<img src={file.url} alt={file.filename} class="w-full h-full rounded-md object-cover" />
							{:else}
