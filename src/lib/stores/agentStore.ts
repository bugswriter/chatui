...
function createAgentStore() {
	...
	async function initialize() {
		...
		try {
			...
			agentsList.forEach(agent => agentMap.set(agent.name.toLowerCase(), agent));

			set({ agents: agentsList, agentsByName: agentMap, isInitialized: true, error: null });
		} catch (e) {
	...
	function search(query: string): Agent[] {
		...
		return agents.filter((agent) => agent.name.toLowerCase().includes(lowerCaseQuery));
	}
